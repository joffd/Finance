namespace Finance.Lib.Tools
    ...
    module Date =
        ...
        let wkendSet =
            [DayOfWeek.Saturday; DayOfWeek.Sunday] |> Set.ofList
            
            
        /// Print a Tenor type (returning a string)
        let printTenor (t: Tenor) =
            "Years: " + t.years.ToString() + " | Months: " + t.months.ToString() + " | Days: " + t.days.ToString()

        /// Check Date (and not time) equality
        let public EqualsDate (d1: DateTime) (d2: DateTime) =
            match (d1.Year.Equals(d2.Year), d1.Month.Equals(d2.Month), d1.Day.Equals(d2.Day)) with
            | (true, true, true) -> true
            | _true              -> false
        ...
        /// Produce a date after adding a Tenor
        let DateFromTenor (d: DateTime) (t: Tenor) =
            d.AddYears(t.years).AddMonths(t.months).AddDays(float t.days)
        ...
        /// Returns Set of Business Dates
        let buildListValidDate rule cal (sd: DateTime) (ed: DateTime) =
            //let sd = sd.Date
            //let ed = ed.Date
                            
            let firstd = sd |> roll rule cal
            let ed = ed |> roll RollRule.Previous cal

            let rec addDaysToSeq cal ed (sd: List<DateTime>) =
                match sd.Head.Equals(ed) with
                | true -> sd
                | false -> ([(rollBy 1 RollRule.Following cal sd.Head)] @ sd) |> addDaysToSeq cal ed

            match (firstd.CompareTo(ed)) with
            | i when i > 0 -> None
            | 1            -> [firstd] |> Some
            | _            -> addDaysToSeq cal ed [firstd] |> List.rev |> Some

        /// Create a Calendar when given holidays (Set of DateTime) and an optional different week-end set of DayOfWeek
        let createCalendar (diffwkend: Set<DayOfWeek> option) hol  =
            { weekendDays = match diffwkend with
                            | Some wk -> wk
                            | None -> wkendSet
              holidays = hol }


        /// Calculates the first Business Day of the next month using a calendar
        let calcFirstBDayNextMonth (cal: Calendar) (d: DateTime) =
            
            let nextM = d.AddMonths(1).Month

            let rec f (m: int) (cal: Calendar) (d: DateTime) =
                match (d.Month = m) with
                | true  -> d
                | false -> f m cal (rollBy 1 RollRule.Following cal d)
        ...
        let rollBack1BDay cal (d: DateTime) =
            roll RollRule.Previous cal (d.AddDays(-1.))
        ...
        let createListDatesAllNotHol (sd: DateTime) (ed: DateTime) (scal: seq<Calendar>) =
